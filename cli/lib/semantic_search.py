...
class SemanticSearch:
    ...
    def generate_embedding(self, text):
        if not text or not text.strip():
            raise ValueError("Cannot generate the embeddings for empty text")
    ...
    def search(self, query, limit : float = DEFAULT_SEARCH_LIMIT):
...
def fixed_size_chunking(text :str, chunk_size : int = DEFAULT_CHUNK_SIZE, overlap : int = DEFAULT_CHUNK_OVERLAP) -> list[dict]:
    words = text.split()
    chunks =[]
    ...
    while i<n_words:
        ...
        if chunks and len(chunk_size) <= overlap:
            break
...
def semantic_chunk(text :str, max_chunk_size : int = DEFAULT_SEMANTIC_CHUNK_SIZE, overlap : int = DEFAULT_CHUNK_OVERLAP) -> list[str]:
    ...
    while i<n_sentences:
        ...
        cleaned_sentances = []
        for chunk_sentance in chunk_sentences:
            cleaned_sentances.append(chunk_sentance.strip())

        if not cleaned_sentances:
            continue
        
        chunk = " ".join(cleaned_sentances)
        chunks.append(chunk)
